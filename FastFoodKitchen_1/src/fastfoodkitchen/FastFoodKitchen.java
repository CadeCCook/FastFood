...
public class FastFoodKitchen {
    ...
    private final ArrayList<BurgerOrder> orderList = new ArrayList();
    private final ArrayList<BurgerOrder> completed = new ArrayList();

    private static int nextOrderNum = 1;

    FastFoodKitchen() throws FileNotFoundException{
        for(ArrayList<String> order : CSVreader.startFile())
        {
            addOrder(Integer.parseInt(order.get(0)), Integer.parseInt(order.get(1)), Integer.parseInt(order.get(2)), Integer.parseInt(order.get(3)), Integer.parseInt(order.get(4)), Boolean.parseBoolean(order.get(5)));
        }
    }
    ...
    public int addOrder(int ham, int cheese, int veggie, int soda, boolean toGo){
        orderList.add(new BurgerOrder(ham, cheese, veggie, soda, toGo, orderList.get(orderList.size()-1).getOrderNum()+1));
        return orderList.get((orderList.size()-1)).getOrderNum();
    }
    
    public int addOrder(int orderNum, int ham, int cheese, int veggie, int soda, boolean toGo) {
    ...
    public ArrayList<BurgerOrder> getCompleted() {
        return completed;
    }
